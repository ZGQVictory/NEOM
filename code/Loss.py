...
def compute_loss(iedb_freqmat, prob_ref_freqmat, out_filename, pep_seq, logfile, w1 = 10, w2 = -0.1, w3 = None, w4 = args.weight_iedb):
    ...
    print('Notice, modeller loss = w1 * best_molpdf + w2 * best_DOPE --> = {} * {} + {} * {}'.format(w1, best_molpdf, w2, best_DOPE), file = logfile)
    if args.cdr_sequence == None:
        print('****cdr loss --> {} * {} = {} given no cdr'.format(w3, cdr_loss, w3 * cdr_loss), file = logfile)
    else:
        print('****cdr loss --> {}* {} = {} with cdr sequence as {}'.format(w3, cdr_loss, w3 * cdr_loss, args.cdr_sequence), file = logfile)
    print('****iedb loss --> {} * {} = {}'.format(w4, iedb_loss, w4 * iedb_loss), file = logfile)
