...
### Additional part ###
from addition_func import get_freq, normalize
    ...
    def generate_loc(self, loc, seq=None, booster_num=args.booster_num):
        ...
        if seq == None:
            ...
            if args.add_booster:
                ...
                for i in loc:  # i is the peptide amino acid positions
                    seq_i = seq[i]
                    prob = self.freq[:, i].copy()
                    index = list(AA_freq.keys()).index(seq_i)
                    # set the corresponding amino acid probability to be zero
                    prob[index] = 0.0
                    prob = [pprob / sum(prob) for pprob in prob]
